      ...
      },
      {
        ...
        cell: (info) => (
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            {info.row.original.isEmailVerified ? (
              <svg
        ...
        cell: (info) => {
          return (
            <div style={{ fontSize: '12px', display: 'grid' }}>
              ...
              <div style={{ color: '#6b7280' }}>{info.row.original.relativeTime}</div>
            </div>
          );
        },
        size: 110,
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((row) => {
      ...
      return (
        ...
        row.npn?.toString().includes(query)
      );
    });
  }, [data, searchQuery]);
    ...
    >
      <Card>
        <CardHeader className="flex items-center justify-between">
          {/* Left side: Title */}
          <CardTitle>Showing {showingUsers} of {totalUsers} users</CardTitle>
