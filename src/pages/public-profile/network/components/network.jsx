...
import { getUplines } from '@/utils/agentService';

const Network = ({ setDropdown, setShowEdit, setUplineId }) => {
  ...
  const handleAutoFetch = () => {
    getUplines()
      .then((response) => {
        const agentsArray = response.data.uplines || [];
        const updated = agentsArray.map(item => ({
          ...item,
          avatar: {
            className: 'size-20 relative',
            image: item?.profile ?? 'default-profile.jpeg',
  ...
  const renderItem = (item, index) => {
    ...
    return (
      ...
      >
        {/* Checkbox Icon */}
        <div onClick={(e) => { e.stopPropagation(); navigate(`/agent/Uplines/${item.id}`); }}>

          <input
            type="checkbox"
            onClick={(e) => { e.stopPropagation(); toggleSelect(index, item) }}
            checked={isSelected}
            readOnly
          ...
          <CardConnection
            name={item.name}
            info={item.website_url}
            avatar={item.avatar || '/media/avatars/default-profile.jpeg'}
            email={item.email}
            team={item.team}
